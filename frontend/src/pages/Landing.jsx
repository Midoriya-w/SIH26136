...
import { DOMAIN_CATEGORIES, PROCESS_FLOW_STEPS } from '../data/mockData';
import { ArrowRight, CheckCircle2, Cpu, GraduationCap, HeartPulse, Landmark, Play, Rocket, ShieldCheck, Sprout, Building2, Leaf, Bus, UserCheck, Briefcase, Files, Scale, Route, Trees } from 'lucide-react';
...
const CountUp = ({ value, prefix = '', suffix = '', decimals = 0 }) => {
  ...
  return <strong ref={numberRef}>{prefix}{formattedValue}{suffix}</strong>;
};
...
const Landing = ({ currentUser, onRoleChange }) => {
  ...
  const serviceGateways = [
    ...
    { label: 'Priority sectors', icon: Files, to: '/startup/challenges' },
  ...
  return <div className="app-container landing-page">
    ...
    <main>
      ...
      <section className="stat-band"><div className="shell stats-grid"><div className="stat"><CountUp value={42} /><span>active challenges</span></div><div className="stat"><CountUp value={1280} /><span>startups onboarded</span></div><div className="stat"><CountUp value={96} /><span>pilots completed</span></div><div className="stat"><CountUp value={18.4} prefix="₹" suffix=" Cr" decimals={1} /><span>pilots contracted</span></div></div></section>
      {/* SIH rule: problem, uniqueness and prototype details are separate equal blocks. */}
      <section className="shell section-space why-section"><div className="section-heading"><p className="eyebrow">Why Samarth</p><h2>A clear path from need to pilot.</h2></div><div className="three-card-grid"><article className="info-card problem"><span>01</span><h3>Problem to pilot</h3><ul><li>Define <b>outcomes</b>, not long specs.</li><li>Test focused, low-risk pilots.</li><li>Keep every stage visible.</li></ul></article><article className="info-card innovation"><span>02</span><h3>What makes it fair</h3><ul><li><b>Three experts</b> score each proposal.</li><li>Conflict-aware allocation.</li><li>Proof before major spend.</li></ul></article><article className="info-card prototype"><span>03</span><h3>Built into the portal</h3><ul><li>Role-based workspaces.</li><li>Fixed scores and feedback.</li><li>One view of pilot progress.</li></ul></article></div></section>
      <section className="shell section-space portal-section"><div className="section-heading row-heading"><div><p className="eyebrow">Three roles</p><h2>Choose your workspace.</h2></div><button className="btn btn-outline" onClick={() => setLoginModalOpen(true)}>Sign in</button></div><motion.div className="role-portal-grid" variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>{roles.map(({ key, title, icon: Icon, label, points }) => <motion.article className={`role-portal-card ${key}`} key={key} variants={reveal} transition={{ type: 'spring', stiffness: 180, damping: 20 }}><div className="role-icon"><Icon size={22} /></div><p className="role-label">{title}</p><h3>{label}</h3><ul>{points.map(point => <li key={point}><CheckCircle2 size={16} />{point}</li>)}</ul><button className="card-link" onClick={() => openPortal(key)}>Open <ArrowRight size={17} /></button></motion.article>)}</motion.div></section>
      {/* The delivery path is revealed progressively as visitors scroll through it. */}
      <section className="workflow-wrap roadmap-wrap" id="workflow"><div className="shell section-space"><div className="section-heading centered"><p className="eyebrow">How it works</p><h2>From challenge to scale.</h2><p>Scroll to follow each stage of the journey.</p></div><div className="roadmap" aria-label="Eight-stage innovation workflow">{PROCESS_FLOW_STEPS.map((step, index) => <motion.article className={`roadmap-item actor-${step.actor.toLowerCase().replaceAll(' ', '-')}`} key={step.step} initial={{ opacity: 0, y: 42, scale: 0.96 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, amount: 0.4 }} transition={{ duration: 0.48, delay: index % 2 ? 0.08 : 0, ease: [0.16, 1, 0.3, 1] }}><div className="roadmap-pin"><span>{step.step}</span></div><div className="roadmap-card"><small>{step.actor}</small><h3>{step.title}</h3><p>{step.desc}</p></div></motion.article>)}</div></div></section>
